...
const deletePost = async (req, res, next) => {
  ...
  if (!req.user.isAdmin || req.user.id != req.params.userId) {
    return next(ApiError(403, 'You are not allowed to delete this post.'))
  }
  ...
  // res.status(200).json("getting response");
}

const updatePost = async (req, res, next) => {
  //console.log("hi");
  if (!req.user.isAdmin || req.user.id != req.params.userId) {
    return next(ApiError(403, 'You are not allowed to update this post'));
  }
  try {
